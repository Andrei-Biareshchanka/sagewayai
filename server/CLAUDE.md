...
| `Situation` | `id`, `slugRu`/`slugEn` (unique, per-locale — the slug is the SEO keyword being targeted, same reasoning as `Parable.slugRu/slugEn`), `h1Ru/En`, `introRu/En`, `metaDescriptionRu/En`, `isPublished` (default `false`). Hand-curated SEO hub pages (`/[locale]/situacii/[slug]`) grouping several `Parable`s under one real search query (e.g. "how do I know if I made the right decision") — not AI-clustered; see `docs/audit-content-model.md` §5(c) for why `Quote.theme`/embeddings weren't used. Written the same way as parable insights: manually, via an Opus subagent (`docs/manual-backfill-process.md`), not a dedicated generation function. |
| `SituationParable` | `id`, `situationId`, `parableId`, `position` (`@@unique([situationId, position])`, editorial ordering — strongest parable first, not alphabetical), `noteRu?/En?` — join table for the many-to-many between `Situation` and `Parable`, modeled directly on `ParableQuote`. The per-pair curator note (why *this* parable answers *this* situation) is the page's highest-value copy — it's what keeps a hub from being a plain link list. |
...
3. If no — `createDigestForDate()` picks the parable and quote (parable-first pipeline — see "Daily digest parable/quote selection" below), builds `conclusion`/`question`/`title` straight from the parable's own already-generated fields (**no Claude call** — see "Reflections and titles" below), generates the slug, creates the record with `isPublished: true` (this is an on-demand creation for *today*, not a draft).
...
**As of 2026-08-02, `createDigestForDate()` makes zero Claude calls.** `buildReflections(parable, timesShown)` reads `conclusion`/`question`/`title` straight off the already-selected `DailyParableCandidate` instead of generating a fresh short reflection per quote pairing:
- `conclusionEn`/`conclusionRu` = the parable's own P0 deep-insight `conclusionEn`/`conclusionRu` (see "Canonical parable insight generation" below) — the same essay shown on `/pritcha/[slug]`, not a new one.
- `questionEn`/`questionRu` = one of the parable's 3 pre-written `questionsEn`/`questionsRu`, picked via `pickQuestion()` at index `timesShown % 3` — the same rotation index `findQuoteForParable()` uses for the quote, so the two rotate in lockstep with zero extra state.
- `titleEn`/`titleRu` = the parable's own `title`/`titleRu` (falls back to `title` if `titleRu` is null) — fixed and stable, no uniqueness check needed since it isn't freshly rolled text.
...
- `selectDailyParable()`: LRU by last-shown date (`MAX(DailyDigest.date)` per parable, `NULLS FIRST` so a never-shown parable always wins over any shown one), under a cooldown exclusion, **restricted to `reflectionStatus = 'REVIEWED'`** — required since `buildReflections()` (`dailyDigest.ts`) now reads `conclusionEn/Ru`/`questionsEn/Ru` straight off the returned candidate (see "Reflections and titles" above) and those fields are only guaranteed non-null on REVIEWED parables. `COOLDOWN_STEPS = [60, 45, 30, 21, 14, 7, 0]` — tries each window in descending order, only relaxing to a shorter one when the stricter window has zero candidates (same defensive-relaxation pattern as the old quote steps below). `0` disables the cooldown entirely, so the loop is guaranteed to terminate with a result as long as a REVIEWED `Parable` exists; throws if none do.
- `getTimesShown(parableId)`: counts every `DailyDigest` row ever created for that parable (drafts included) — the single source of truth for both rotations below. Must be called *before* today's `DailyDigest` row is created — counting it would shift both rotations by one.
- `findQuoteForParable(parable, timesShown)`: rotates each parable through its 3 pre-assigned `ParableQuote` rows (backfilled once, positions 0/1/2, one `isPrimary`) instead of re-running a vector search on every digest. `position = timesShown % 3`. Throws if a parable is missing a `ParableQuote` at the expected position (a real data problem, not a soft-fallback case). `dailyDigest.ts`'s `pickQuestion()` rotates through the parable's 3 questions using the same `timesShown` value, so the quote and question stay in lockstep.
